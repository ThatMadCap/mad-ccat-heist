...
function createMathBoard() {
    ...
    for (let i = 0; i < 7; i++) {
        for (let j = 0; j < 7; j++) {
            if (i == 0 || i == 6) {
                if (i==0){
                    mathEl += j % 2 ? `<div class="math-grid-item math-answer" style="background-color: transparent;color: transparent;user-select: none;" data-answer="A${answerNum++}"></div>`: `<div class="math-grid-item math-empty"></div>`;
                }else{
                ...
                if (j == 0 || j == 6) {
                    if (j == 0) {
                        mathEl += `<div class="math-grid-item math-answer" style="background-color: transparent;color: transparent;user-select: none;" data-answer="A${answerNum++}"></div>`;
                    }else{
...
function randomOps() {
    ...
    const opsArr = [];
    for (let i = 0; i < 12; i++) {
        let randomOp = ops[Math.floor(Math.random() * operatorCount)];
...
function startMathGame(settings) {
    activeGame = "math";
    operatorCount = settings.operators;
    createMathBoard();
